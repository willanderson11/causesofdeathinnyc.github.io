...
## Motivation to Investiagate HIV-Related Deaths
...
## Visualizing HIV Incidence and Mortality Rates by Borough, Race, and Gender


```{r}
#Average Number of HIV-related Deaths in NYC (2011-2015)

hiv_data %>% 
  filter(borough == "All") %>% # Filter hiv data by borough
  select(.data = ., year, deaths)   %>%
      group_by(year) %>%
         summarise(mean_deaths = mean(deaths)) %>% #mean number of deaths 
 ggplot(aes(x = year, y = mean_deaths)) +
    geom_line() +
    geom_point()+
  labs (title = "Decline in Number of HIV-related deaths in NYC (2011-2015)",
    x = "Year",
...
```{r}
#Average HIV-related deaths in NYC by race 
...
hiv_data %>% # Filter hiv data by borough
  ...
  select(.data = ., year, deaths, race, borough)   %>% 
      group_by(year, race, borough) %>%
         summarise(mean_deaths = mean(deaths)) %>% #mean number of deaths  
 ggplot(aes(x = year, y = mean_deaths, color = race)) +
    geom_line() +
  facet_grid(~borough) +
  labs (title = "Average HIV-related deaths in NYC by Race (2011-2015)",  
    x = "Year",
    y = "Average Number of deaths") + 
...
```{r}
#Average Number of HIV-related deaths in NYC by Sex 
...
hiv_data %>% 
  ...
  select(.data = ., year, deaths, sex, borough)   %>% 
      group_by(year, sex, borough) %>%
         summarise(mean_deaths = mean(deaths)) %>% #mean number of deaths  
 ggplot(aes(x = year, y = mean_deaths, fill = sex)) +
  geom_bar(stat= "identity") +
  facet_grid(~borough) +
  labs(title = "Average HIV-related deaths in NYC by Sex (2011-2015)",  
    x = "Year",
    y = "Average Number of deaths") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  theme(plot.title = element_text(hjust = 0.5))  
...
```{r}
# Total Number of Deaths in NYC by Age 
hiv_data %>% 
  filter(age != "All") %>% 
  ...
  labs(x = "Age Groups",
       y = "Number of Deaths",
       title = "HIV-related deaths in NYC by Age (2011-2015)") +
  theme(plot.title = element_text(hjust = 0.7))  

```

```{r}
# Total Number of Hiv-Related Deaths in NYC by Race 
hiv_data %>% 
  filter(race != "All") %>% 
